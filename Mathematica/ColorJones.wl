...
It carries out the majority of algorithm 2 and equation (9.2) from Hajij, Levitt arXiv: ____.____v1 .
It should be used when the faster compiled form doesn't work with your version of Mathematica or your system lacks a C-compiler. *)
...
(*  The implementation below is the first of two separate implementations.
	It follows the second description of Algorithm 4 in Hajij, Levitt arXiv: ____.____v1 .
	It should be used when the faster compiled form doesn't work with your version of Mathematica or your system lacks a C-compiler. *)
...
(*  The implementation below is the second of two separate implementations.
	It roughly follows the description of Algorithm 4 in Hajij, Levitt arXiv: ____.____v1 .
...
(* This is an indentical but more readable version of the actual function call. *)
...
{stackHeight=1, loopDone=False, CJP=1, mirror=False, CJtemp, TPW, Ncrs, walks, simpleWalks, mirrorSimpleWalks, numStrands, writhe, LNG, aList, bList, cList, CoeffList, walkWeights, NCvars, i, j, localBraid, mirrorLocalBraid},
...
(*Print["Length: ",Length[simpleWalks[[2]]]," mirror: ",Length[mirrorSimpleWalks[[2]]]];*)
...
(* We collect the q terms to ensure usable output.
   This doubles runtime on average, but is much more efficient than ExpandAll[] or Apart[] *)
CJP=ParallelCombine[Collect[#,CJP`q]&,CJP,DistributedContexts->None];
CJP=Collect[CJP*(CJP`q^(((NC-1)*(writhe-numStrands+1))/2)),CJP`q];

(*CJP=Apart[ParallelCombine[Collect[#,CJP`q]&,CJP,DistributedContexts\[Rule]None]];*)
